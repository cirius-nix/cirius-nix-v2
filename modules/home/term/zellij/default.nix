...
in {
  options.${namespace}.term.zellij = {
    enable = mkEnableOption "Enable zellij terminal workspace";
    leader = lib.${namespace}.strlib.mkOption "Ctrl a" "Leader key for zellij";
  };
  config = let
    inherit (lib) mkIf;
    cfg = config.${namespace}.term.zellij;
    mkBindAct = shortcuts: actionName: action: {
      "bind \"${shortcuts}\"" = {
        "${actionName}" = action;
      };
    };
    mkBindsAct = list: actionName: action:
    ...
    mkBindMoveFocus = shortcuts: direction:
        ...
        settings = {
          ...
          "keybinds clear-defaults=true" = {
            locked = lib.foldl' lib.recursiveUpdate {} [
              (mkBindSm cfg.leader "Normal")
            ];
            normal = lib.foldl' lib.recursiveUpdate {} [
              (mkBindsSm [cfg.leader "ESC" "Enter" "Space"] "Locked")
              (mkBindSm "p" "Pane")
              (mkBindSm "m" "Move")
              (mkBindSm "t" "Tab")
              (mkBindSm "r" "Resize")
